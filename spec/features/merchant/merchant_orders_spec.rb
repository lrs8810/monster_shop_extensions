...
describe 'As a logged in Merchant (employee/admin)' do
  before(:each) do
    ...
    @order = @user.orders.create!(name: 'Bob', address_id: @home_address.id, status: 'Pending')
  ...
  it 'I only see the items in the order that are being purchased from my merchant' do
    ...
    expect(page).to have_content(@home_address.address)
    expect(page).to have_content(@home_address.city)
    expect(page).to have_content(@home_address.state)
    expect(page).to have_content(@home_address.zip)
