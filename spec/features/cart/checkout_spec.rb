...
RSpec.describe 'Cart show' do
  describe 'When I have added items to my cart' do
    ...
    it 'If I am a registered user, but I don\'t have any addresses tied to my profile there is no link to checkout but a prompt to create an address' do
      user = User.create(
        name: 'Bob',
        email: 'bob@email.com',
        password: 'secure'
      ...
      expect(page).to have_content('Please add address to checkout')

      within '#checkout' do
        click_link 'add address'
      end

      expect(current_path).to eq(profile_new_address_path)
    end
  end
