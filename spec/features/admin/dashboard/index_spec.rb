...
RSpec.describe 'As an admin user' do
  describe 'when I visit my dashboard' do
    before(:each) do
      ...
      @home_address = @user_1.addresses.create(address: '123 Main', city: 'Denver', state: 'CO', zip: 80_233)

      @order_1 = @user_1.orders.create(name: 'User 1', address_id: @home_address.id, status: 2)
      @order_1.item_orders.create(order_id: @order_1.id, item_id: @pull_toy.id, quantity: 1, price: 100, merchant_id: @dog_shop.id)

      @order_2 = @user_1.orders.create(name: 'User 2', address_id: @home_address.id, status: 3)
      @order_2.item_orders.create(order_id: @order_2.id, item_id: @pull_toy.id, quantity: 1, price: 10, merchant_id: @dog_shop.id)

      @order_3 = @user_1.orders.create(name: 'User 1', address_id: @home_address.id, status: 1)
      @order_3.item_orders.create(order_id: @order_3.id, item_id: @pull_toy.id, quantity: 2, price: 15, merchant_id: @dog_shop.id)

      @order_4 = @user_1.orders.create(name: 'User 2', address_id: @home_address.id, status: 0)
